...
class PlayState extends MusicBeatState
{
	...
	var rompBopper:FlxSprite;
	var rompBopper2:FlxSprite;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'betrayed':
			{
				...
				var bg:FlxSprite = new FlxSprite(-210, -208).loadGraphic(Paths.image('romp/rompBG'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.updateHitbox();
				add(bg);

				rompBopper = new FlxSprite(-183.2, 115.6);
				rompBopper.frames = Paths.getSparrowAtlas('romp/romp_bop');
				rompBopper.antialiasing = true;
				rompBopper.scrollFactor.set(0.7, 0.7);
				rompBopper.animation.addByPrefix('bop', 'Romp boppers', 24);
				rompBopper.updateHitbox();
				add(rompBopper);

				rompBopper2 = new FlxSprite(978.75, 115.6);
				rompBopper2.frames = Paths.getSparrowAtlas('romp/romp_bop');
				rompBopper2.antialiasing = true;
				rompBopper2.scrollFactor.set(0.7, 0.7);
				rompBopper2.animation.addByPrefix('bop', 'Romp boppers', 24);
				rompBopper2.flipX = true;
				rompBopper2.updateHitbox();
				add(rompBopper2);
		...
		switch (SONG.player2)
		{
			...
			case 'romp':
				dad.x -= 50;
				dad.y += 202;
				FlxG.watch.add(dad, 'x');
				FlxG.watch.add(dad, 'y');
		}
	...
	*/
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'betrayed':
				if(FlxG.save.data.distractions){
					rompBopper.animation.play('bop', true);
					rompBopper2.animation.play('bop', true);
				}
